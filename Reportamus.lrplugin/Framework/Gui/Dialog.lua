...
function Dialog:messageWithOptions( message, ... )
    ...
    if buttons then
        ...
        elseif type( buttons ) == 'string' then
            args.cancelVerb = '< exclude >' -- works on Mac too? ###1 (reliable on Windows?).
...
--
function Dialog:buttons( shortcut )
...
--
function Dialog:promptForTargetPhotos( prefix, returnComponents, call, viewItems, accItems )

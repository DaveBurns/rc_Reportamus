...
function View:getThumbnailViewItem( params )
    ...
    if app:lrVersion() >= 4 then
        ...
        local vi = vf:static_text( params.viewOptions )
        if params.viewOptions.height == nil then
            params.viewOptions.height_in_lines = 5
        end
...
--          <br>initNewPreset ( function( presetId, presetName, presetKey ) - returns s, m; required ) initialize data associated with specified preset.
...
function View:makePresetPopup( params )
    ...
    local s, m = app:call( Call:new{ name="View - Make Preset Popup", async=false, guard=App.guardNot, main=function( icall )
        ...
        local function computePopupItems( presetNames, presetName )
            ...
            if found then
                if str:isStartingWith( found, "__" ) then
        ...
        -- called in response to edited change handler.
        ...
        local function popupChgHdlr( one, two, id, value )
            ...
            app:call( Call:new{ name="Popup Change Handler", async=true, guard=App.guardSilent, main=function( icall )
                    ...
                    repeat
                        ...
                        if s then
                            ...
                            if s then
                                ...
                                props.popupSel = prevPresetName -- perhaps theoretically an attemp should be made to reload, as is done in case of preset-name selection. ###1
                            end
                        else
                ...
                if value == '__save_as__' or value == '__save__' then
                    ...
                    if s then
                        if value == '__save_as__' then
                            ...
                            local newNames = callback.getPresetNames( presetId, presetNames, s ) or error( "no names" )
                            --Debug.pause( "setting", presetNamesPref, #newNames, "names for save-as" )
                            app:setPref( presetNamesPref, newNames ) -- save preset names.
                            app:setPref( selectedPresetNamePref, s ) -- save preset name selected.
                            computePopupItems( newNames, s ) -- will set popup-sel to match preset-name.
                            props.edited = false
                            app:show{ info="Saved edited '^1'", s, actionPrefKey = "Saved preset" }
                        end
                    else
                ...
                elseif not str:isBeginningWith( value, "__" ) then -- preset name selection
                    ...
                    -- Note: perhaps an easier way to deal with this is to have items where value includes prefix, but title doesn't ###1.
        ...
        args.tooltip = args.tooltip or "Feel free to name the current dataset, for future reference, by choosing 'Save As'. Note: you can rename the set by choosing 'Delete' followed by 'Save As' - nothing will be lost."
...
function View:getPresetName( presetId, props )
    ...
    if str:is( presetName ) then
        if props.popupSel == presetName then -- steady state
            return presetName, self:getPresetKey( presetId, presetName )
        else
            --Debug.pause( props.popupSel, presetName ) - generally (always probabl), preset-name is the incoming, and popup-sel is the outgoing, thus
            -- it may make more sense to simply return the incoming ###1.
            return nil -- in transition.
        end
    else
        if not str:is( props.popupSel ) then

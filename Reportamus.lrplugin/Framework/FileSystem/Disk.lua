...
function Disk:deleteFileConfirm( path )
    if self:existsAsFile( path ) then
        ...
        repeat
            ...
            LrFileUtils.delete( path )
            count = count + 1
        until shutdown or count >= 100 -- ten seconds max.
        if shutdown then
            return
        else
...
Disk.deleteFile = Disk.deleteFolderOrFile -- synonym
Disk.deleteFolder = Disk.deleteFolderOrFile -- synonym
...
function Disk:makeReadWrite( path )
    ...
    if WIN_ENV then
        ...
        --s = LrFileUtils.makeFileWritable( path ) -- ###1 trying @20/Oct/2012 17:54, until 8/Jan/2013 18:13 (ChangeManager error solved be reverting to original, dunno cause for sure - not repeatable).
        if not s then
...
function Disk:copyTree( src, dest, excl, overwrite, timeCheckIsEnough, call )
    ...
    if not LrFileUtils.isEmptyDirectory( src ) then
        ...
        app:log( "Empty dir - skipped: ^1", src ) -- ###1
    end
    if errs > 0 then
...
function Disk:isChangedSince( srcFile, lastMod )
    ...
    local d1 = LrFileUtils.fileAttributes( srcFile ).fileModificationDate
    return d1 > ( lastMod or -math.huge ), d1
...
--- @deprecated - Same as lr-file-utils--exists
--

...
function Preferences:loadPrefFile( file, presetName )
    ...
    if filePrefs then
        ...
        assert( dir == self.setsDir, "bad dir" )
    end
        ...
        if prefTbl then
            if type( prefTbl ) == 'table' then
                if filePrefs then
                    ...
                    -- note: this was a pretty big bug, probably need to re-release some plugins ###1.
                    local preset = self.presetCache[presetName]
                    if preset then
...
function Preferences:loadPrefSupportFile( presetName )
    ...
    if file then
        if fso:existsAsFile( file ) then
            self:loadPrefFile( file, presetName ) -- throws error if problems.
        else
...
function Preferences:getPref( propName, presetName )
    ...
    if not str:is( presetName ) or presetName == 'Default' then
        ...
        if preset.backingData then -- note: @20/May/2013 6:28, backing file is not being reloaded by standard reload scheme, thus old (cached) data is being returned from previously read backing file ###1
...
function Preferences:makePresetPopup( params )
    ...
    local function record( v )
        ...
        --props.value = v             -- what was this for?, not sure @20/Jan/2013 5:24 ###1 - doesn't seem to hurt anything, so leaving in (cant tell benefit either in present situation testing). - leftover from days before value was bound to sel?
        -- trying without (oh boy) @20/Jan/2013 5:34 - maybe not a good idea... - watch it...
        
        --vw.value = v                -- what was this for? @20/Jan/2013 5:02 - not sure ###1. removed 20/Jan/2013 5:30, since it makes the menu shaky, hopefully no longer needed!?
                    ...
                    local function loadBacker( soBacked, backingDir, dfltFile )
                        ...
                        if file then
                            if fso:existsAsFile( file ) then
                                ...
                                -- local dir = LrPathUtils.parent( file )
                                -- ###1: bug - if preset is cached, then it's not being loaded into 'backingData'.
        	            ...
        	            if self.setsBacking then
        	                ...
        	                if dataDescr then
                                ...
                                if params.props then
        	                        self:loadDefaults( params.props, presetName ) -- props ignored if nil. ###1 callback if no props?

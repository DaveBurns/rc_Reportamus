...
--  @param          noQuotes        (boolean, optional) set true to keep from over-wrapping with quotes. ###1
...
function App:changeFileDates( params )
    ...
    else -- ###1 @17/Dec/2012 20:46, not yet tested on Mac.
...
function App:call( op, ... )
    ...
    return op.status, op.message -- I like this, but not compatible with previously defined return code.
end
...
--             app:dispErr{ disp="display me", log="log ^1", "me" } -- called using condensed synonym, and using table params instead. Note: varargs are intra-table, and apply to log msg (no fmtx subs for disp msg).
--
...
function App:displayError( params, ... )
    ...
    -- return
--[[ was until 9/Apr/2013 12:38 ->    
    if params == nil then
        params = {}
    elseif type( params ) ~= 'table' then
        app:logError( "disp-err method requires table param" )
        return
    end
    if params.log then
        local varargs = {}
        ...
        if str:isString( params.log ) then
            app:logError( params.log, unpack( varargs ) )
        elseif str:is( params.disp ) then
            app:logError( params.disp, unpack( varargs ) )
        else
            app:logError( "An unknown error occurred - no error message provided." )
        end
    end
    app:call( Call:new{ name="Display Error Message", async=true, guard=App.guardSilent, progress={ title=str:fmtx( "*** ^1 - Error *** (click 'x' to clear)", app:getAppName() ) }, main=function( call )
        local disp
        if params.disp == nil then
            disp = "See log file for more info..." -- hopefully error was logged externally, if not internally.
        else
            disp = str:squeezeToFit( params.disp, 55 ) -- Note: Lr truncates. squeeze width is about right on Windows (but depends on chars in disp msg) - dunno bout mac.
        end
        call:setCaption( disp )
        while not call:isQuit() do
            LrTasks.sleep( .2 )
...
App.dispErr = App.displayError -- App:dispErr( ... )
...
function App:displayWarning( params, ... )
    ...
    -- return
--[[ was until 9/Apr/2013 12:39 (never instituted hold-off for warning version, but should have) ->
    if params == nil then
        params = {}
    elseif type( params ) ~= 'table' then
        app:logWarning( "disp-err method requires table param" )
        return
    end
    if params.log then
        local varargs = {}
        ...
        app:logWarning( params.log, unpack( varargs ) )
    end
    app:call( Call:new{ name="Display Warning Message", async=true, guard=App.guardSilent, progress={ title=str:fmtx( "^1 - Warning (click 'x' to clear)", app:getAppName() ) }, main=function( call )
        local disp
        if params.disp == nil then
            disp = "See log file for more info..." -- hopefully warning was logged externally, if not internally.
        else
            disp = str:squeezeToFit( params.disp, 55 ) -- Note: Lr truncates. squeeze width is perfect on Windows (maybe depends on chars in errm) - dunno bout mac.
        end
        call:setCaption( disp )
        while not call:isQuit() do
            LrTasks.sleep( .2 )
...
App.dispWarn = App.displayWarning -- App:dispWarn( ... )
...
function App:sleepUnlessShutdown( time, incr, doneFunc )
    ...
    incr = math.min( incr or .1, 1 ) -- 1 second is minimum increment, 100msec or shorter (for short intervals) is recommended.
    repeat
        LrTasks.sleep( incr )
...
App.sleep = App.sleepUnlessShutdown -- App:sleep( ... )

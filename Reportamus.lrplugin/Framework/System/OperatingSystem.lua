...
--
--  @param              command (string, required) Complete command string.
--  @param              expectedReturnCode (number, default=0) expected return code for successful operation.
--
...
function OperatingSystem:execute( command, expectedReturnCode )
    ...
    if sts then
        if other ~= nil and other == expectedReturnCode then
            ...
            return false, str:fmtx( "Invalid exit code - expected ^1, but ^2 was returned by command: ^3", expectedReturnCode, str:to( other ), command )
        end
    else

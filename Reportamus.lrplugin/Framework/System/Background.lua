...
local Background, dbg = Object:newClass{ className = 'Background' } -- only one error id used by base class. derived classes can have additional error IDs if desired.
...
--  @param      call - call object containing background context.
--
...
--
function Background:displayError( errm, id )
    ...
    if call.nErrorsDisplayed > 1 then
        Debug.pause( "###1 ERROR: background error", errm ) -- pseudo error. 
    ...
    LrTasks.startAsyncTask( function()
        ...
        call.errScope = LrProgressScope {
            title = str:fmtx( "^1 - background error", app:getAppName() ),
        ...
        call.errScope = DelayedProgressScope:new { -- changed to delayed-scope 22/Oct/2012 14:20, so fleeting errors don't cause a stir (e.g. when a photo is deleted).
            ...
            caption = app:parseErrorMessage( errm ),
            functionContext = call.context,
            delaySecs = self.interval <= 1 and 3 or self.interval * 3,
            updateSecs = self.interval,
            -- @22/Oct/2012 14:55, default is non-modal.
        }
        app:sleep( math.huge, .1, function()
            if call.errScope and call.errScope:isCanceled() then
...
function Background:considerIdleProcessing( call )
    ...
    if not doTargetPhotos and not doAllPhotos and not doFilmstripPhotos then
        dbg( "No idle processing is enabled." )
        return
    end
    ...
    if doFilmstripPhotos then
        if app:getPref( 'processVisiblePhotosInBackground' ) ~= nil then
            filmstripPhotos = cat:getVisiblePhotos() -- yum.
        else
...
function Background:main( call )
    ...
    LrTasks.sleep( .1 ) -- without this ya cant see the startup progress bar. ###1 consider a hold-off time, maybe randomized, so all catalog-intesive plugins are not contending simultaneously.

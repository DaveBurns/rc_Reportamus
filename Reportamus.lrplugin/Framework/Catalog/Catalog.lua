...
function Catalog:withRetries( tryCount, func, p1, p2, ... )
    ...
    while( true ) do
        for i = 1, tryCount do
            local sts, qual = LrTasks.pcall( func, catalog, p1, p2, ... ) -- what other parameters could there be? ###1 8/Apr/2013 21:15, context?
            if sts then
                return true, qual
            elseif str:is( qual ) then
                local found = qual:find( "LrCatalog:with", 1, true ) or 0 -- return position or zero, instead of position or nil.
...
function Catalog:_update( catFunc, tmo, name, func, ... )
    ...
    repeat -- do all phases.
        local s, other
        if app:getLrVerMajor() < 4444444 then -- ###3 Lr4 timeout handling is buggy - catastrophically in my case, sincing phasing progression depends on reliable/consistent error handling,
            ...
            --s = true -- ###1 discovered and removed 28/Jan/2013 18:17 - seemed wrong! (it seems I maybe remember doing this because of some problem with a plugin(s) 
            ...
            if _s then -- access granted, function completed.
                ...
                other = _m
                s = false -- added 28/Jan/2013 18:28 ###1 remove this comment and the above if no problems come 2014.
            end
        else
...
function Catalog:readPhotoMetadata( photo, photoPath, alreadyInLibraryModule, service, manualSubtitle )
    ...
    local mode
    if WIN_ENV or app:lrVersion() >= 5 then -- mac supports read-metadata via sdk in Lr5+. ###1 may need to change other places to allow.
...
function Catalog:restoreSelPhotos( selPhotos )
    ...
    local s, m = LrTasks.pcall( function()
        ...
        if selPhotos.selectedPhotos and #selPhotos.selectedPhotos > 0 then
            assert( selPhotos.mostSelPhoto ~= nil, "sel photos without most-sel" )
...
--- Get list of photos in filmstrip. *** ###1 deprecated in favor of getVisiblePhotos, which could stand to be augmented with parameters like includeIfBuried... - exclude source would not fit, and perhaps should be handled as special case in 1 plugin using it: MissingInAction.
    ...
    if s then
        ...
        repeat
            ...
            elseif button == 'cancel' then
                call:cancel() -- ###1 not necessarily true (need to pass a "mandatory" flag maybe(?)).
                return
            else
                error( "bad button" )
            end
        ...
        if WIN_ENV then
            ...
            -- local s, m = app:sendMacEncKeys( "CmdOptionShift-Delete" ) -- ###1 not tested (I believe the modifiers are correct, the 'Delete' part is a guess).
        end
        return true
    else
        app:logError( m )
...
--[[  Determine if specified photo is locked, or not. *** NYI
--  @usage prefer the batch version if more than one photo.
function Catalog:isLockedViaChangeManager( photo )
    local changeManagerId = app:getPref{ name='changeManagerId', default='com.robcole.lightroom.ChangeManager' } -- for checking lock status.
    if str:is( changeManagerId ) then
        local meta = photo:getRawMetadata( 'customMetadata' ) -- different format than batch mode.
...
--
function Catalog:isLocked( photo )
    ...
    if str:is( changeManagerId ) then
        ...
        if cdata then
            return cdata.locked == 'yes', cdata.lockDate -- else not locked.
        end
    end
end

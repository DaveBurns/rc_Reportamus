...
--[[
        ...
        ###1 - consider constraining booleans too for friendly value.
--]]
DevelopSettings.table = { -- static table with all info for all settings.
    ...
    {   groupName="Basic",
        members={
            ...
            { members = { -- anonymous sub-group
                { id='Temperature', friendly="Temperature (Raw)", dataType = 'number', baseAdj=50, constraints = { min=-10000, max=10000 }, prereq = { name='WhiteBalance', value='Custom' } },
    ...
    {   groupName = "Tone Curve",
        members={
            -- constraints are for absolute values, but relative sliders must be able to go incrementally negative too. ###1
            ...
            -- ###1: need to define relative constraints independently of absolute constraints, somehow.
    ...
    {   groupName = "Lens Corrections",
        members = {
            { members={ -- profile-based
                -- { id='LensProfileEnable', friendly="Lens Profile Enable", dataType = 'number', constraints = { 0, 1 }, -- until 5/Feb/2013 17:56 ###1 expected in some contexts, no doubt.
...
-- popuplate settings-lookup table and popup-items array.
...
--
--  @usage Does *NOT* assure pre-requisites. ###1
...
--
--  @usage Does *NOT* assure pre-requisites. ###1 - also consider pv mismatch
...
function DevelopSettings:applyAdjustments( params )
    ...
    return app:call( Call:new{ name="Develop Settings - Apply Adjustments", guard=App.guardNot, main=function( call )
        ...
        -- otherwise, applying auto-toning in conjunction with other basic adjustments (or non-basic?) will fail. ###1
        ...
        local function applyAdj()
            ...
            for i, adjRec in ipairs( adjRecs ) do
                repeat
                    local photo = adjRec.photo
                    local isVideo = lrMeta:getRawMetadata( photo, 'fileFormat', cache, true ) == 'VIDEO' -- accept uncached.
                    if isVideo then
                        app:logv( "Develop settings do not apply to videos, ignoring: ^1", lrMeta:getRawMetadata( photo, 'path', cache, true ) )
                        break
                    end
                    prog( i - 1, #adjRecs )
                    local settings
                    if preConstrained then
                        settings = adjRec.settings
                    else
                        settings = self:constrainSettings( adjRec.settings )        
                    end
        ...
        --local s, m = cat:update( 20, "Evaluating/Applying Pre-requisite Settings", function( context, phase )
            -- Note: could do 100 or 1000 at a time, like cookmarks, but really: are people going to be adjusting multi-thousands at once? (maybe) ###1
            applyPrereq()
        --end )
        --if s then

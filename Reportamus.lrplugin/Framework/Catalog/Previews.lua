...
local levelToWidth = {
    80, 160, 320, 640, 1280, 2560, 99999 -- ###1 works?
}
local levelToHeight = {
...
-- returns nil, errm or table (see return statement for format).
function Previews:_getPreviewInfo( photo )
    ...
    --[[ Note: after running this test, *all* images had only one line for each image id, meaning only one digest.
    -- yet, I've seen an additional digest created when changes made, that's how I learned to program for freshness==3 (assuring recent changes had settled
    -- into previews. Assuming I'm not crazy, it must be a temporary state - after a while new previews get folded into stead-state previews.
    local n0, n1, n2 = 0, 0, 0
    app:call( Call:new{ name="test", async=false, guard=App.guardNot, progress=true, main=function( call )
        local t = str:fmt( "select * from ImageCacheEntry" )
        local sts, cmdOrMsg, dat = sqlite:executeCommand( param, { t }, nil, 'del' )
        local lines = str:split( dat, "\n" )
        for i, line in ipairs( lines ) do
            call:setPortionComplete( i - 1, #lines )
            repeat
                if not str:is( line ) then
                    break
                end
                local comp = str:split( line, '|' )
                if #comp ~= 4 then
                    Debug.pause( "not 4", #comp )
                    break
                end
                local id = comp[1]
                ...
                local t = str:fmt( "select * from ImageCacheEntry where imageId=^1", id )
                local sts, cmdOrMsg, dat = sqlite:executeCommand( param, { t }, nil, 'del' )
                local lns = str:split( dat, "\n" )
                if not str:is( lns[#lns] ) then
                    lns[#lns] = nil
                end
                if #lns > 1 then
                    n2 = n2 + 1
    ...
    if sts then
        if str:is( dat ) then
            ...
            if #c >= 3 then
                ...
                orientationCode = c[3]
                if #c > 3 then
                    Debug.pause( imageId, #c )
                else
                    --Debug.pause( imageId, uuid, digest )
                end
            else
...
function Previews:_getImageDataFromFile( previewSourcePath, minLevel, prefLevel )
    ...
    local function assureTerm( content )
        ...
        assert( #lastChar == 1, "###1" )
        local lastChar2 = content:sub( -2, -2 )
        assert( #lastChar2 == 1, "###1" )
        if lastChar == string.char( 0xD9 ) then
            if lastChar2 == string.char( 0xFF ) then
                app:logV( "File content is properly terminated." )
                return content, 0 -- already terminated.
            else
                app:logV( "Last block ends with 0xD9, but is not preceded by 0xFF - strange but not an error: appending termination block." )
                return content .. string.char( 0xFF ) .. string.char( 0xD9 ), 2
            end
        elseif lastChar == string.char( 0xFF ) then -- mode of failure experienced by Jean Chang.
            app:logV( "Last char is 0xFF - appending 0xD9 for proper termination." )
            return content .. string.char( 0xD9 ), 1
        else
            app:logV( "File content is not properly terminated, appending termination block." )
    ...
    while level <= prefLevel do
        ...
        if termCount == 0 then
            app:log( "Preview terminated as read." )
        elseif termCount == 1 then
            app:log( "Preview, as read, was only partially terminated." )
        elseif termCount == 2 then
            app:log( "Preview, as read, was not terminated." )
        else
...
--  @param freshness (number, default=1) 0 => whatever is handy; 1 => use dates to determine freshness (*** recommended ***); 2 => freshen regardless of dates; 3 => must be hot off the press.
...
function Previews:getImage( photo, photoPath, previewFile, prefLevel, minLevel, freshness, icc, profile, meta, orient, mogParam, fmtMeta, ets, assureDir, dontOverwrite, tryNewLr5WayFirst )
    ...
    if previewFile == nil then -- handle virtual copy better? ###2 - perhaps I shoudn't even support getting previews for virtual copies,
        ...
        if isVirt then
            ...
            -- previewFilename = LrPathUtils.leafName( photoPath ) -- ###1 - how have I gotten by with this - filename should be jpg. 26/Jun/2013 5:15
    ...
    if tryNewLr5WayFirst and app:lrVersion() >= 5 then
        ...
        app:sleep ( 5, .05, function()
            return imageData
        end )
        if shutdown then return nil, "shutdown" end
    end

    -- most of these not needed below next clause ###1
    local imageId -- this one *is* needed.
    local cd
    local n
    local pn
    local d
    ...
    if imageData then
        ...
        cd = previewInfo.catDir
        n = previewInfo.catName
        pn = n .. " Previews.lrdata"
        d = LrPathUtils.child( cd, pn )
        uuid = previewInfo.previewId
        digest = previewInfo.previewDigest
        orientationCode = previewInfo.previewOrient
        pdb = previewInfo.previewDb
        
        previewSubdir = str:getFirstChar( uuid )
        pDir = LrPathUtils.child( d, previewSubdir )
        if fso:existsAsDir( pDir ) then
            -- good
        else
            return nil, "preview letter dir does not exist: " .. pDir
        ...
        if image then
            ...
            if icc or meta then
                if newDigest == nil then
                    ...
                    sourceProfile = 'sRGB' -- ###1 not sure if color-profile is correct or not yet @26/Jun/2013 6:58.
                    sourceProfile = 'AdobeRGB'
                else
    ...
    -- assure freshness at "3".
    local function assureFreshness()
        local previewFilename = str:fmtx( "^1-^2.lrprev", uuid, digest )
        local previewSourcePath = LrPathUtils.child( pDir, previewFilename )
    ...
    if freshness == 3 then -- must be *very* fresh.
        local fresh, msg = assureFreshness()
        if not fresh then
...
--
--  @usage not supporting cache param just yet.
--
...
function Previews:getSmartPreview( params ) -- the easy way
    ...
    local spi = lrMeta:getRawMetadata( photo, 'smartPreviewInfo', cache, true ) -- accept uncached.
    if spi then

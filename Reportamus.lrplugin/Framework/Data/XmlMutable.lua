--[[
        XmlMutable.lua *** DEPRECATED IN FAVOR OF XML-BUFFER, because it's problematic to set attrs externally based on lookup, since
                        xml is serialized based on array, thus lookup changes will be ignored. XmlBuffer offers the ability to go through
                        methods for everything, so nothing is left to chance...
...
local XmlMutable, dbg = Object:newClass{ className = 'XmlMutable', register = true }



--- Constructor for extending class.
--
function XmlMutable:newClass( t )
...
function Attributes:new( attrTable )
    ...
    o.array = attrTable
    o.lookup = {} -- beware that this lookup is for read-only.
    if attrTable then
        for i, v in ipairs( attrTable ) do
            o.lookup[v.name] = v.text
        end
    end
    return o
end
...
--
--  @usage attribute MUST already exist.
...
function Attributes:setAttrValue( name, value )
    if value ~= nil then
        if self.lookup[name] ~= nil then
            self.lookup[name] = str:to( value )
        else
            app:callingError( "Use add-attr to add attributes." )
        end
    else
...
--  @param value (any, required) any serializable value (serialized by global lua tostring function, tostring method in metatable, or toString class method if available).
--
--  @usage attribute must NOT already exist.
--
...
--
--  @return array (table(array)) items are tables with named items: name/text.
function Attributes:getArray()
    return self.array
end
...
function Attributes:serialize( ind )
    local b = {}
    for i, v in ipairs( self.array ) do -- ###1 - this is a bug, since attr values changed in lookup table aren't reflected in array - fixed in XmlBuffer.
        b[#b + 1] = "\n" .. ind .. v.name .. '="' .. v.text .. '"'
    end
    if #b > 1 then
...
Element.getChildAt = Element.getChildAtIndex -- synonym



--- Gets attributes as array.
--
--  @return attribueArray (table(array)) empty if no attributes, else elements are tables containing name/text members.
--
function Element:getAttributeArray()
    return self.attrs:getArray()
end
...
--- Get attributes as lookup table.
--
--  @return attributeLookupTable (table(named members)) key is attribute name, value is string.
--
function Element:getAttributeLookup()
    return self.attrs:getLookup()
end
Element.getAttributes = Element.getAttributeLookup -- default/synonym.
...
function Element:serialize( initialIndentation, spacesPerIndent )
    local function _serialize( node, indent, spaces ) -- serialize element node
        ...
        if str:is( t ) and #node == 0 then
            s = s .. '>' .. t .. '</' .. name .. ">\n"
        else -- ###1 note: although this creates legal xml, it won't work for gradients in xmp, since the "short form" is required for empty elements. - use XmlBuffer instead.
            s = s .. '>\n'
...
--                              
--
function XmlMutable:_parseAttrs(s)
  local args = {}
...
--                              - empty = 1, or text.
--
function XmlMutable:_tabularize(s)
    local stack = {}
...
--
function XmlMutable:_serializeAttributes( attrs )
    local args = ''
...
function XmlMutable:_spaces( level )
...
function XmlMutable:_escape( original )
...
--  Presently only used for lua serializer, although could be adapted for general use.
--
function XmlMutable:_unescape( escaped )
...
--		                    - recursive.
--
function XmlMutable:_serializeLua( lua, indentLevel, attrStr )
    local s
...
--
function XmlMutable:newDocument( xml, includePreamble )
    if xml ~= nil then
...
--
function XmlMutable:serializeLua( luaT, includeDecl, nSpaces )
...
--
function XmlMutable:parseToLua( xmlString, expectDecl )
    ...
    local function convertElement( elem, luaT )
        ...
        local lookup = elem:getAttributeLookup()
        local value
        local index
        if not tab:isEmpty( lookup ) then
            local index_text = lookup['i']
...
--
function XmlMutable:parseXml( x, decl )
...
--
function XmlMutable:serialize( xdoc, initialIndentationSpaceString, numberOfSpacesPerIndentLevel )
